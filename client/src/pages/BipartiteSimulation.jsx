...
export default function BipartiteSimulation() {
  ...
  const [bipartiteState, setBipartiteState] = useState({
    queue: [],
    color: {},
    currentNode: null,
    currentNeighbors: [],
  ...
  useMemo(() => {
    ...
    stopSimulation();
    executorRef.current = new BipartiteExecutor(adj, start);
    setBipartiteState(executorRef.current.getState());
  }, [input]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-24">
        <div className="max-w-6xl mx-auto space-y-6">
          <div>
            <h1 className="text-4xl font-bold text-foreground">Bipartite Graph Check</h1>
            <p className="text-muted-foreground mt-2">
              2-Coloring using BFS - Check if graph can be divided into two sets
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-card border border-border rounded-lg p-4">
                ...
                <div className="flex gap-3">
                  ...
                  <Button onClick={step} variant="outline" className="flex-1" disabled={isSimulating || bipartiteState.done}>
                    Step
                  </Button>
                  <Button 
              ...
              <div className="bg-card border border-border rounded-lg p-4">
                ...
                <div className="min-h-10 flex items-center gap-2 flex-wrap">
                  {bipartiteState.queue.length ? (
                    bipartiteState.queue.map((q, i) => (
                      <div
                        key={`${q}-${i}`}
              ...
              <div className="bg-card border border-border rounded-lg p-4">
                ...
                <div className="max-h-60 overflow-auto text-sm font-mono">
                  {bipartiteState.log.length ? (
                    bipartiteState.log.map((entry, i) => (
                      <div
                        key={i}
                        className={`py-1 ${
                          entry.startsWith("  ")
            ...
            <div>
              <GraphVisualization
                adjacencyList={adj}
                visitedNodes={Object.keys(bipartiteState.color).filter(node => bipartiteState.color[node] !== -1)}
                queuedNodes={bipartiteState.queue}
                currentNode={bipartiteState.currentNode}
                currentNeighbors={bipartiteState.currentNeighbors}
                width={600}
                height={500}
              />
            </div>
